...
class SpoolUsedLength(CoordinatorEntity, SensorEntity):
    """Sensor for spool used length - reduces state attributes on main sensor."""

    def __init__(
        self, hass, coordinator, spool_data, config_entry
    ) -> None:
        """Initialize the used length sensor."""
        ...
        self.entity_id = generate_entity_id(
            "sensor.{}",
            f"spoolman_spool_{spool_data['id']}_used_length",
            hass=hass
        )
    ...
    @property
    def state(self):
        """Return the used length in mm."""
        return round(self._spool.get("used_length", 0), 2)
