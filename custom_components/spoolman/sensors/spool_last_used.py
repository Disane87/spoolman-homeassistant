...
class SpoolLastUsed(CoordinatorEntity, SensorEntity):
    """Sensor for spool last used timestamp."""
    ...
    def __init__(self, hass, coordinator, spool_data, config_entry) -> None:
        ...
        self.entity_id = generate_entity_id("sensor.{}", f"spoolman_spool_{spool_data['id']}_last_used", hass=hass)
        self._attr_unique_id = f"spoolman_{self._entry.entry_id}_spool_{spool_data['id']}_last_used"
        self._attr_has_entity_name = False
        self._attr_name = f"{spool_name} Last Used"
    ...
    @property
    def state(self):
        return self._spool.get("last_used")
