"""Sensor class: FilamentArticleNumber."""
...
class FilamentArticleNumber(CoordinatorEntity, SensorEntity):
    """Sensor for filament article number."""
    ...
    def __init__(self, hass, coordinator, spool_data, config_entry) -> None:
        ...
        self.entity_id = generate_entity_id("sensor.{}", f"spoolman_spool_{spool_data['id']}_article_number", hass=hass)
        self._attr_unique_id = f"spoolman_{self._entry.entry_id}_spool_{spool_data['id']}_article_number"
        self._attr_has_entity_name = False
        self._attr_name = f"{spool_name} Article Number"
    ...
    def state(self):
        filament = self._spool.get("filament", {})
        return filament.get("article_number")
