...
class VendorName(CoordinatorEntity, SensorEntity):
    """Sensor for vendor name."""
    ...
    def __init__(self, hass, coordinator, spool_data, config_entry) -> None:
        ...
        self.entity_id = generate_entity_id("sensor.{}", f"spoolman_spool_{spool_data['id']}_vendor", hass=hass)
        self._attr_unique_id = f"spoolman_{self._entry.entry_id}_spool_{spool_data['id']}_vendor"
        self._attr_has_entity_name = False
        self._attr_name = f"{spool_name} Vendor"
    ...
    def state(self):
        filament = self._spool.get("filament", {})
        return filament.get("vendor", {}).get("name")
